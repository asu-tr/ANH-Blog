    ...
    public class BlogDbInitializer : CreateDatabaseIfNotExists<BlogDbContext>
    {
        protected override void Seed(BlogDbContext context)
        {
            ...
            for (int i = 1; i < userCount; i++)
            {
                User u = new User()
                {
                    Username = FakeData.NameData.GetFullName().Trim().ToLower(),
            ...
            for (int i = 0; i < postCount; i++)
            {
                ...
                Post p = new Post()
                {
                    Title = FakeData.TextData.GetSentences(i),
                    Text = FakeData.TextData.GetSentences(15),
                    IsDraft = draft,
                    Category = context.Categories.Find(rnd.Next(0,categoryCount)),
                    User = context.Users.Find(rnd.Next(0,userCount))
                };
            ...
            for (int i = 0; i < 50; i++)
            {
                Comment comment = new Comment()
                {
                    ...
                    User = context.Users.Find(rnd.Next(0, userCount)),
                    Post = context.Posts.Find(rnd.Next(0, postCount))
                };
